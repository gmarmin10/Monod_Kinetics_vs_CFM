...
Note:
Here I use different font from Tricho
...
#     print("YdnaC_N",YdnaC_N)
    ...
    if Plot_C==1:
        ...
        if healey85data.i == 3 and What_is_limiting == 0:
            plot(Dd,BiomassC, color=plotcolor,label=str(I)+'$\mu$E m$^{-2}$s$^{-1}$',zorder=-1)
            plot(healey85data.DL, healey85data.L,'o',color=plotcolor,zorder=0)
        else:
            plot(Dd,BiomassC, color=plotcolor,label=str(I)+'$\mu$E m$^{-2}$s$^{-1}$')
            plot(healey85data.DL, healey85data.L,'o',color=plotcolor)
          
        xlabel('$\mu$ (d$^{-1}$)', fontsize = AxisFontSize)
        ylabel('mg C L$^{-1}$', fontsize = AxisFontSize)
    ...
    if Plot_PC==1:    
        ...
        line=plot(Dd,PtoCplot, color=plotcolor,label=str(I)+'$\mu$E m$^{-2}$s$^{-1}$',zorder=ord)
        ...
        xlabel('$\mu$ (d$^{-1}$)', fontsize = AxisFontSize)
        ylabel('mol P mol C$^{-1}$', fontsize = AxisFontSize)
        #title('P-C ratio')
    #     ylim(ymin=0)
    ...
    if Plot_NC==1:
        ...
        if healey85data.i == 3 and What_is_limiting == 0:
            plot(Dd, NtoCplot, color=plotcolor,label=str(I)+'$\mu$E m$^{-2}$s$^{-1}$',zorder = -1)
            plot(healey85data.D_NtoC, healey85data.NtoC/NunitData,'o' ,color=plotcolor,zorder = 0)
        else:
            plot(Dd, NtoCplot, color=plotcolor,label=str(I)+'$\mu$E m$^{-2}$s$^{-1}$')
            plot(healey85data.D_NtoC, healey85data.NtoC/NunitData,'o' ,color=plotcolor)
        
        xlabel('$\mu$ (d$^{-1}$)', fontsize = AxisFontSize)
        ylabel('mol N mol C$^{-1}$', fontsize = AxisFontSize)
        ylim(ymin=0)
        xlim(xmax=xmax0)
        if What_is_limiting==0:
...
#########################
    if Plot_NP==1:
        ...
        if What_is_limiting==0:
            ...
            ZorderFactor = -1
        #--------------------------------------------------------------------
        plot(Dd, NtoPplot, color=plotcolor,label=str(I)+'$\mu$E m$^{-2}$s$^{-1}$',zorder=1000-I*ZorderFactor)
        plot(healey85data.D_NtoP, healey85data.NtoP/NunitData*PunitData,'o' ,color=plotcolor,zorder=2000-I*ZorderFactor)
        xlabel('$\mu$ (d$^{-1}$)', fontsize = AxisFontSize)
        ylabel('mol N mol P$^{-1}$', fontsize = AxisFontSize)
        ylim(ymin=0)
        xlim(xmax=xmax0)
        if What_is_limiting==0:
            ylim(ymax=140)  
...
###########################
    ...
    if Plot_Chl==1:
        figure(7)
        plot(Dd,ChltoCplot,color=plotcolor,label=str(I)+'$\mu$E m$^{-2}$s$^{-1}$')
        ...
        xlabel('$\mu$ (d$^{-1}$)', fontsize = AxisFontSize)
        ylabel('$\mu$g chlorophyll a mg C$^{-1}$', fontsize = AxisFontSize)
...
from Dmax_computation18 import *
...
Cphoto=Dmax0.Cphoto
Cbiosynth=Dmax0.Cbiosynth
Cconst_protein=Dmax0.Cconst_protein
...
Cphoto[Dmax<0]=nan
Cbiosynth[Dmax<0]=nan
Cconst_protein[Dmax<0]=nan
...
if What_is_limiting==0:
    Dmaxdata=genfromtxt('../data/Healey_data_Dmax_for_AW_method_P_limited.csv',delimiter=',')
if What_is_limiting==1:
    Dmaxdata=genfromtxt('../data/Healey_data_Dmax_for_AW_method_N_limited.csv',delimiter=',')
...
xlabel('Irradiance $\mu$mol m$^{-2}$ s$^{-1}$', fontsize = AxisFontSize)
ylabel('$\mu_{max}$ (d$^{-1}$)', fontsize = AxisFontSize)
legend(loc=4,fontsize=30,borderaxespad=1)
savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Healey85\\Mumax.png",dpi=DPI)
 
figure(3)
plot(Dmax*86400,BiomassC,':',color='black')
savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Healey85\\C concentration.png",dpi=DPI)
 
figure(5)
plot(Dmax*86400,PtoCplot/PunitData,':',color='black',zorder=1000)
ylim(top=0.014)
savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Healey85\\P to C.png",dpi=DPI)
 
figure(6)
plot(Dmax*86400,NtoCplot/NunitData,':',color='black',zorder=-1)
savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Healey85\\N to C.png",dpi=DPI)
 
figure(7)
plot(Dmax*86400,ChltoCplot,':',color='black',zorder=-1)
savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Healey85\\Chl to C.png",dpi=DPI)
...
savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Healey85\\N to P.png",dpi=DPI)

#---preparing computing Cphoto and Cbio---

# Cprotein = Cphoto + Cbiosynth + Cconst_protein
# rCphoto = Cphoto/Cprotein 
# rCbiosynth = Cbiosynth/Cprotein
# rCconst_protein = Cconst_protein/Cprotein
# 
# figure(11)
# plot(Dmax*86400,rCphoto,'orange',label='Photo')
# plot(Dmax*86460,rCbiosynth+rCconst_protein,'green',label='Bio + Other')
# ylim(ymin=0)
# xlabel('$\mu_{max}$ (d$^{-1}$)')
# ylabel('Protein mass fraction')
# 
# savefig("C:\\Users\\Keiin\\OneDrive\\desktop\\figures\\01\\Phytoplankton model\\"+Whatislimiting+"\\Healey85\\Protein Mass Fraction.png",dpi=DPI)
# legend(loc = 4,fontsize=24)
#------------------------------------------
